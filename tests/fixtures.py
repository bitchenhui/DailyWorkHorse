...
# 全部 31 个申万一级行业，按净流入降序——真实采集出来就是这个形状，
# 板块段要让它们同场赛跑，所以夹具也必须给满，否则测不出 31 行的排版。
#
# 煤炭与基础化工的形状是刻意配的：前者全天垫底、最后两分钟才超过后者。
# 这个尾盘反超正是「定格帧停在换位半路上」的复现条件。
SECTORS = (
    ("农林牧渔", 22.5e8, 0.8), ("银行", 12.4e8, 1.2), ("公用事业", 8.7e8, 0.7),
    ("石油石化", 4.1e8, 1.0), ("交通运输", 3.3e8, 1.5), ("房地产", 2.0e8, 0.5),
    ("煤炭", 1.8e8, 27.0), ("基础化工", 0.8e8, 0.4), ("钢铁", 0.5e8, 1.1),
    ("环保", 0.2e8, 0.9), ("综合", -0.3e8, 1.3), ("美容护理", -0.9e8, 0.6),
    ("纺织服饰", -1.4e8, 1.0), ("社会服务", -2.1e8, 1.7), ("建筑材料", -2.8e8, 0.8),
    ("商贸零售", -3.6e8, 1.4), ("轻工制造", -4.5e8, 0.5), ("家用电器", -5.4e8, 1.1),
    ("国防军工", -7.2e8, 2.2), ("建筑装饰", -8.9e8, 0.7), ("传媒", -11.3e8, 1.0),
    ("汽车", -14.6e8, 1.6), ("食品饮料", -18.2e8, 0.9), ("机械设备", -23.5e8, 1.2),
    ("医药生物", -29.4e8, 0.6), ("非银金融", -35.1e8, 1.8), ("有色金属", -44.8e8, 0.6),
    ("电力设备", -45.9e8, 2.0), ("计算机", -66.6e8, 1.1), ("通信", -111.6e8, 0.9),
    ("电子", -172.3e8, 1.4),
)
STOCK_INFLOW = (
